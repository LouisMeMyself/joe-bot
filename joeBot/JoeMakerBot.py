...
class JoeMaker:
    def __init__(self):
        ...
        # contracts
        self.joeMakerV1 = w3.eth.contract(
            address=Constants.JOEMAKERV1_ADDRESS, abi=Constants.JOEMAKERV1_ABI
        )
        self.joeMakerV2 = w3.eth.contract(
            address=Constants.JOEMAKERV2_ADDRESS, abi=Constants.JOEMAKERV2_ABI
        )
        self.joeMakerV3 = w3.eth.contract(
            address=Constants.JOEMAKERV3_ADDRESS, abi=Constants.JOEMAKERV3_ABI
        )
    ...
    def execContract(self, func_):
        ...
        gas = int(
            float(func_.estimateGas({"from": self.account.address}))
            * (1 + Constants.PREMIUM_PER_TRANSACTION)
        )
        if gas > Constants.MAX_GAS_PER_BLOCK:
            raise Exception("Max gas per block reached")
        construct_txn = func_.buildTransaction(
            {"from": self.account.address, "nonce": nonce, "gas": gas}
        )
    ...
    def _callConvertMultiple(self, groups_tokens0, groups_tokens1, error_on_pairs):
        ...
        for group_tokens0, group_tokens1 in zip(groups_tokens0, groups_tokens1):
            call_convert_multiple = self.joeMaker.functions.convertMultiple(
                group_tokens0, group_tokens1
            )
            ...
            try:
                ...
                self._callConvert(
                    group_tokens0, group_tokens1, pairs, joe_bought_back, error_on_pairs
                )
    ...
    def callConvertMultiple(self, min_usd_value):
        ...
        tokens0, tokens1 = JoeSubGraph.getJoeMakerPostitions(
            min_usd_value, self.joeMaker.address
        )
        ...
        groups_tokens0, groups_tokens1 = getGroupsOf(safe_tokens0), getGroupsOf(
            safe_tokens1
        )

        # return 0, 0, 0
        # calls ConvertMultiple with the previously grouped tokens
        pairs, joe_bought_back, error_on_pairs = self._callConvertMultiple(
            groups_tokens0, groups_tokens1, error_on_pairs
        )
    ...
    def decodeTxHash(self, tx_hashs):
        ...
        JM_ADDRESS_256 = "0x000000000000000000000000{}".format(
            self.joeMaker.address[2:].lower()
        )
        ...
        for tx_hash in tx_hashs:
            ...
            for i in range(len(logs)):
                if len(logs[i]["topics"]) > 2:
                    if (
                        logs[i]["topics"][1].hex() == JM_ADDRESS_256
                        and logs[i]["topics"][2].hex() == JM_ADDRESS_256
                        and len(logs[i]["data"]) == 64 * 2 + 2
                    ):
                        pairTokens = [
                            getSymbolOf(token)
                            for token in getPairTokens(logs[i].address)
                        ]
                    ...
                    if logs[i]["topics"][2].hex() == ZERO_ADDRESS_256:
                        if logs[i - 1]["topics"][1].hex() == ZERO_ADDRESS_256:
                            ...
                            shift = 0
                        if i > 2:
                            joe_bought_back.append(
                                int("0x" + logs[i - shift - 2]["data"][-64:], 16) / 1e18
                            )
                        nb_tokens += 1
...
def decodeTransactionReceipt(
    transaction_receipt, tokens0, tokens1, joe_bought_back, pairs
):
    ...
    for i in range(len(logs)):
        if (
            len(logs[i]["topics"]) > 2
            and logs[i]["topics"][2].hex() == ZERO_ADDRESS_256
        ):
            if logs[i - 1]["topics"][1].hex() == ZERO_ADDRESS_256:
                shift = 1
            else:
            ...
            try:
                pairs.append(
                    "{} - {}".format(
                        getSymbolOf(tokens0[nb_tokens]), getSymbolOf(tokens1[nb_tokens])
                    )
                )
            except IndexError:
