PROFILE_PICTURE_COMMAND = "!joepic"
JOEPIC_CHANNEL_NAME = "profile-pictures"
JOESUGGEST_CHANNEL_NAME = "💡-ideas-and-feedback"
JOEREACT_CHANNEL_NAME = "📚-guidelines-and-resources"

GUIDELINES_MSG_ID = (852636768788021288,)
emoji = None
NAME_OF_EMOJI = 'joe'
CHECK = "✅"
CROSS = "❌"

ADMIN_CHANNEL_NAME = "🤖-bot-commands"

ROLE_FOR_CMD = "Bot Master"
ROLE_TO_VIEW = "Farmer"
ERROR_ON_JOEPIC = """How to use joeBot for profile pictures:

1. Choose a HEX color or a RGB color in these formats: '#00FFFF', '00FFFF', '0 255 255' or '0,255,255'
https://htmlcolorcodes.com/color-picker/

2. Enter this command `!joepic [color]` with your selected color

3. Save image + add as your Discord profile photo

Users with a Joe profile picture will be tagged in a @Joe  role!

If you want to suggest a feature or anything for joebot, use `!suggest [your suggestion here]`!"""


